...
# MAJ .htaccess for admin domain and public domain
if [ -z ${DOMAINE_ADMIN+x} ]; then
	echo >&2 "WARNING: DOMAINE_ADMIN environment variable is not defined";
else
	echo "var DOMAINE_ADMIN is set to '$DOMAINE_ADMIN'";
fi
if [ -z ${DOMAINE_PUBLIQUE+x} ]; then
	echo >&2 "WARNING: DOMAINE_PUBLIQUE environment variable is not defined";
else
	echo "var DOMAINE_PUBLIQUE is set to '$DOMAINE_PUBLIQUE'";
fi
if [ ! -z ${DOMAINE_ADMIN+x} ] && [ ! -z ${DOMAINE_PUBLIQUE+x} ]; then
	# Création du fichier .htaccess avec les URL admin <> publique
	sed -e "s/\[DOMAINE_ADMIN\]/${DOMAINE_ADMIN}/g" .htaccess-sample |sed -e "s/\[DOMAINE_PUBLIQUE\]/${DOMAINE_PUBLIQUE}/g">.htaccess
	echo ".htaccess created with DOMAINE_ADMIN=${DOMAINE_ADMIN} and DOMAINE_PUBLIQUE=${DOMAINE_PUBLIQUE}"
else
	# Création du fichier .htaccess avec les URL admin = publique : suppression de la config domaine admin
	sed '/## BEGIN Domaine Admin/,/## END Domaine Admin/d' .htaccess-sample>.htaccess
	echo ".htaccess created without DOMAINE_ADMIN=${DOMAINE_ADMIN} and DOMAINE_PUBLIQUE=${DOMAINE_PUBLIQUE}"
fi
